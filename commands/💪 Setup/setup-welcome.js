    ...
    try {
      ...
      async function first_layer(){
        ...
        collector.on('collect', async menu => {
          ...
          else menu?.reply({content: `<:no:951013282607685632> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:950884027320135711> **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
