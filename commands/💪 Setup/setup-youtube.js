    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Youtube-Poster', 'https://cdn.discordapp.com/emojis/950886430421418004.png?size=128', 'http://discord.gg/7PdChsBGKd'))
        ...
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `:white_check_mark: **Selected: \`${collected && collected.first() ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {
          ...
          case "Edit Youtube Channel":{
            ...
            let MenuEmbed = new MessageEmbed()
              .setColor(es.color)
              .setAuthor('Youtube-Poster', 'https://cdn.discordapp.com/emojis/950886430421418004.png?size=128', 'http://discord.gg/7PdChsBGKd')
