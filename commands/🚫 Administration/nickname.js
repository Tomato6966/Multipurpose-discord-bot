    ...
    try {
      if(!message.guild.me.permissions.has([Permissions.FLAGS.MANAGE_NICKNAMES]))      
        return message.reply({embeds : [new MessageEmbed()
          ...
          .setTitle(`<:no:990786942348193843> **I am missing the Permission to Manage Nicknames of others**`)
        ]})
      ...
      if (!kickmember)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:990786942348193843> **You forgot to ping a Member**`)
      ...
      if(!args[1])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:990786942348193843> **You forgot to add a Nickname**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      ...
      if(nickname.length > 32)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:990786942348193843> **The Nickname must have smaller then 32 Characters**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      kickmember.setNickname(nickname).then(member => {
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.color)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:Yes:964306724821536768> **Successfully changed the username of \`${kickmember.user.tag}\` to \`${nickname}\`**`)
        ]});
      }).catch(e=>{
