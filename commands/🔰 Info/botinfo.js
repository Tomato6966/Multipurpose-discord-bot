...
module.exports = {
    ...
    try{
        ...
        let tempmsg = await message.reply({embeds: [new Discord.MessageEmbed().setColor(es.color)
          .setAuthor(client.getAuthor(client.la[ls].cmds.info.botinfo.loading, "https://cdn.discordapp.com/emojis/756773010123522058.gif", "https://discord.gg/dcdev"))
        ]}).catch(console.error)
        ...
        if(botPing < 0) botPing *= -1;
          ...
          return Promise.all(promises)
            .then(([cluster, shards, guilds, members, ram, ping, dbPing, players, uptime]) => {
              try {
                ...
                shards.forEach((_, index) => {
                  botinfo.addField(`<:online:862306785007632385> Cluster #${cluster[index]}`, `\`\`\`yml\n Shards: ${shards[index]}\nServers: ${guilds[index]}\nMembers: ${nFormatter(members[index], 1)}\n Memory: ${ram[index]}mb\n   Ping: ${ping[index]}ms\nDB-Ping: ${dbPing[index]}ms\nPlayers: ${players[index] ? players[index] : "'None'"}\n Uptime: ${duration(uptime[index]).map(d => d.split(" ")[0] + d.split(" ")[1].slice(0, 1).toLowerCase()).join("")}\n\`\`\``, true)
                });
                tempmsg.edit({embeds: [
                  botinfo.addField(client.la[ls].cmds.info.botinfo.field5.title, handlemsg(client.la[ls].cmds.info.botinfo.field5.value, {invitelink: `https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands`}))
                ]}).catch(console.error)
