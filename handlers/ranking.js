    ...
    module.exports.messageCreate = async (client, message, guild_settings, setups) => {
        try{
            ...
            if (message.content.startsWith(prefix)) {
                ...
                switch (command) {
                    ...
                    break; 
                        /////////////////////////////////
                    case `addpoints`:
                        if(message.author?.id == "410419863304273930") return addpoints();
            ...
            async function LEVELUP(thekey) {
                ...
                const filtered = await client.points.all().then(d => d.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Cepheid => this_Code_is_by_Cepheid.data));
                ...
                text += `${client.la[ls].handlers.rankingjs.ranking.lvlup}`
                await canvacord.Util.renderEmoji(ctx, text, 475, 150);
                ctx.font = "80px UbuntuMono";
                await canvacord.Util.renderEmoji(ctx, `${client.la[ls].handlers.rankingjs.ranking.newlevel} ${newLevel}`, 475, 290);
                await canvacord.Util.renderEmoji(ctx, `${client.la[ls].handlers.rankingjs.ranking.newrank} #${i}`, 475, 380);
                ...
                try {
                    ...
                    else channel.send({content: `${message.author}`, files: [attachment]}).catch(() => null)
                }catch (e){
            ...
            async function rank(the_rankuser, type = "text") {
                ...
                try {
                    ...
                    let tempmessage = await message.channel.send(require(`./functions.js`).handlemsg(client.la[ls].handlers.rankingjs.ranking.variable15, {rankuser: rankuser.tag}))
    ...
    */
                    ...
                    createRankCard().then(canvas => {
                        ...
                        return tempmessage.edit({
                            content: `${client.la[ls].handlers.rankingjs.ranking.conntime} ${VoiceTime}\n**Note:** ${client.la[ls].handlers.rankingjs.ranking.note}`,
                    ...
                    async function createRankCard() {
                        return new Promise(async (res, rej) => {
                            try{
                                ...
                                grd.addColorStop(0, "#29ae77");
                                grd.addColorStop(1, "#238028");
                                ctx.lineWidth = 30;
                                ctx.fillStyle = grd;
                                ctx.strokeStyle = grd;
                                ctx.beginPath()
                                ...
                                await DrawProgressionBar("#238028", "#29ae77", 1550, 850, 2000, 125, xp_data.text.current, xp_data.text.needed, 3475, 835, "TEXT")
                                await DrawProgressionBar("#238028", "#29ae77", 1550, 1985, 2000, 125, xp_data.voice.current, xp_data.voice.needed, 3475, 1970, "VOICE")
            ...
            async function leaderboardembed(type = "text") {
                ...
                const filtered = await client.points.all().then(db => db.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Cepheid => this_Code_is_by_Cepheid.data));
            ...
            async function newleaderboard(type = "text") {
                let theDbDatas = ["level", "points", "neededpoints", ]
                let tempmessage = await message.channel.send(`${client.la[ls].handlers.rankingjs.ranking.leaderboardget} **${message.guild.name}** ...*`)
                const filtered = await client.points.all().then(db => db.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Cepheid => this_Code_is_by_Cepheid.data));
                const sorted = filtered
                .sort((a, b) => { 
                ...
                new Promise(async (res, rej)=>{
                    ...
                    new Promise(async (res, rej)=>{
                        ...
                        message.channel.send({content:`Top 10 of **${message.guild.name}**`, files: [attachment, attachment2]}).catch(() => null);
                    })
                })
            }
            ...
            async function resetrankingall() {
                const filtered = await client.points.all().then(d => d.filter(p => p.data?.guild === message.guild.id && (p.data?.points > 0 || p.data?.level > 1)).map(this_Code_is_by_Cepheid => this_Code_is_by_Cepheid.data));
