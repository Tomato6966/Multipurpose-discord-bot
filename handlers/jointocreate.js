...
module.exports = function (client) {
  ...
  client.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    ) {if(newState.id == "410419863304273930" ) console.log("no new join".bgCyan); return true;}
    ...
    if (!guildData || typeof guildData !== "object") {if(newState.id == "410419863304273930" ) console.log("no data".bgCyan); return }
    // JOINED A CHANNEL
    ...
    async function joinChannel() {
      return new Promise(async (res, rej) => {
        ...
        if(!validEntries || validEntries.length == 0) {if(newState.id == "410419863304273930" ) console.log("no valid entries".bgCyan); return res(true);}
        for await (const [key, value] of validEntries) {
          if (value.channel == newState.channelId) {
            ...
            return res(true);
          } else {if(newState.id == "410419863304273930" ) console.log("not the rright channel".bgCyan); continue;}
        }
        return res(false);
      })
    }
