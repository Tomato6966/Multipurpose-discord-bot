      ...
      try {
        ...
        if (data) {
          if (data.state != "closed") {
            if(!interaction.replied) await interaction.reply({ content: `:x: **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            else await interaction.editReply({ content: `:x: **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            return
          }
        }
      } catch {
