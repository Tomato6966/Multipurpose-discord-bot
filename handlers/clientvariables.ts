...
export default (client: ExtendedClient) => {
   ...
   */
    ...
    client.getInvite = async (id) => {
        ...
        let ch = await client.channels.fetch(id).catch(() => { });
        if (!ch) {
            return `COULD NOT CREATE INVITE FOR: <#${id}>`;
        };

        if (!ch.isTextBased() || ch.isDMBased() || ch.isThread()) {
            return `COULD NOT CREATE INVITE, MAKE SURE CHANNEL IS TEXT BASED!`
        }
        let inv = await ch.createInvite();
        if (!inv) return `COULD NOT CREATE INVITE FOR: <#${id}> in **${ch.guild.name}**`;

        return `<#${id}> | discord.gg/${inv.code}`
    }
