const { edit_Roster_msg, send_roster_msg, delay } = require(`./functions`);

module.exports = client => {
    const maxRoster = 100;

    var CronJob = require("cron").CronJob;
    client.Jobroster = new CronJob(
        "0 */30 * * * *",
        function () {
            client.guilds.cache.map(guild => {
                for (let i = 1; i <= maxRoster; i++) {
                    ...
                    if (
                        thedb?.has(guild.id) &&
                        thedb?.has(guild.id, pre) &&
                        thedb?.get(guild.id, pre + ".rosterchannel") &&
                        thedb?.get(guild.id, pre + ".rosterchannel").length > "notvalid".length
                    ) {
    ...
    client.on("guildMemberUpdate", async function (oldMember, newMember) {
        try {
            ...
            if (rolechanged) {
                ...
                let roleadded = [];
                if (newRoles.length > 0) for (let i = 0; i < newRoles.length; i++) roleadded.push(newRoles[i]);
                //array for removed roles
                let roleremoved = [];
                if (oldRoles.length > 0) for (let i = 0; i < oldRoles.length; i++) roleremoved.push(oldRoles[i]);
