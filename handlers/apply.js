...
module.exports = client => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      for(let i = 1; i<=applySystemAmount; i++) {
        let d = client[`apply${i}`]
        if(d?.has(guild.id) && message.id === d?.get(guild.id, "message_id") && channel.id === d?.get(guild.id, "channel_id")) index = i;
      }
    }
    if(!index) {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction?.customId.startsWith("Apply_")) {
        ...
        for(let i = 1; i<=applySystemAmount; i++) {
          let d = client[`apply${i}`]
          if(d?.has(message.guild.id) && channel.id === d?.get(message.guild.id, "f_channel_id") && d?.has(message.id)) index = i;
        }
        if(!index) {
        ...
        try {
          ...
          if (interaction?.customId == "Apply_accept") {
            ...
            var usert = await client.users.fetch(apply_db.get(message.id, "temp")).catch(() => {});
