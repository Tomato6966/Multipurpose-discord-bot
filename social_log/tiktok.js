...
module.exports = client => {
    ...
    async function check(guild) {
        return console.log(
            ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} :: Currently Disabled the TIKTOK Logger`.italic
                .brightMagenta
        );
        console.log(
            ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Checking Accounts...`.italic
                .brightMagenta
        );
        ...
        try {
            ...
            if (
                !client.social_log.get(guild.id, `tiktok.channels`) ||
                client.social_log.get(guild.id, `tiktok.channels`).length < 1
            ) {
                return console.log(
                    ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Doesn't have TikTok Channels anymore!`
            ...
            var channel =
                guild.channels.cache.get(client.social_log.get(guild.id, `tiktok.dc_channel`)) ||
                (await guild.channels.fetch(client.social_log.get(guild.id, `tiktok.dc_channel`)).catch(() => {})) ||
                false;
            if (!channel)
                return console.log(
                    ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: | No Discord Channel found`
                        .italic.brightMagenta
                );

            client.social_log.get(guild.id, `tiktok.channels`).forEach(async tiktoker => {
                console.log(
                    ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: [${tiktoker}] | Start checking...`
                        .italic.brightMagenta
                );
                try {
                    ...
                    if (!posts.collector[0])
                        return console.log(
                            ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} [${tiktoker}] :: NOT FOUND / No Posts!`
                                .italic.brightMagenta
                        );
                    ...
                    if (client.tiktok.get(tiktoker, `oldvid`) == video.id)
                        return console.log(
                            ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: [${tiktoker}] | Last video is the same as the last saved`
                                .italic.brightMagenta
                        );
                    channel
                        .send({
                            content: client.tiktok
                                .get(tiktoker, `message`)
                    ...
                    console.log(
                        ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Notification sent !`
                            .italic.brightMagenta
                    );
                } catch (error) {
                    console.log(
                        ` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: `.magenta,
                        String(error).grey
                    );
                }
            });
