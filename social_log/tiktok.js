...
module.exports = async (client) => {
    ...
    async function check(guild){
        ...
        try {
            ...
            for(const tt of DBdata.tiktok.channels){
                await dbEnsure(client.tiktok, tt, {
                    oldvid: ``,
                    message: `**{videoAuthorName}** uploaded \`{videoTitle}\`!\n**Watch it:** {videoURL}`
                })
            }
            var channel = guild.channels.cache.get(DBdata.tiktok?.dc_channel) || await guild.channels.fetch(DBdata.tiktok?.dc_channel).catch(() => {}) || false;
